...
const SKIP_PATTERNS = [
  // Totals and subtotals — very aggressive matching
  ...
  /\btot[a4]l\b/i,  // OCR misreads: "tot4l", "totai"
  /\bttl\b/i, /\btotaal\b/i,  // abbreviations and Dutch
  /\bgesamt\b/i, /\bsumme\b/i,  // German
  /\btotale\b/i, /\btotaal\b/i,  // Italian/Dutch
  /\bmontant\b/i,  // French
  /\bto\s*pay\b/i, /\byou\s*paid\b/i, /\bamount\b/i,
  /\bdue\b/i, /\bowed\b/i,
  /\bsubt\b/i,  // abbreviated subtotal
  /\bitems?\s*\d+/i,  // "Items: 12" or "Item 5"
  /\b\d+\s*items?\b/i,  // "12 items"
  /\bnr\s*of\s*items\b/i,
  /\bnum(ber)?\s*of\s*items\b/i,

  // Tax
  /\btax\b/i, /\bvat\b/i, /\bgst\b/i, /\bhst\b/i, /\bpst\b/i,
  /\btax\s*rate\b/i, /\btaxable\b/i, /\bsales\s*tax\b/i,
  /\bmwst\b/i, /\biva\b/i, /\btva\b/i,  // German/Italian/French tax
  /\btax\s*incl/i, /\bincl\s*tax/i,
  /\bexcl\s*tax/i, /\btax\s*excl/i,
...
export function processReceiptText(rawText) {
  ...
  // Post-processing: remove items whose price equals the sum of other items
  // This catches "total" lines that slipped through
  if (unique.length > 2) {
    const allAmounts = unique.map((item) => item.amount);
    const totalSum = allAmounts.reduce((sum, a) => sum + a, 0);

    const filtered = unique.filter((item) => {
      // Check if this item's price equals the sum of ALL other items
      const sumWithout = totalSum - item.amount;
      const diff = Math.abs(item.amount - sumWithout);
      if (diff < 0.05) return false; // This is a total line

      // Check if this item's price is suspiciously large compared to others
      // (more than 60% of the total and there are 3+ other items)
      if (unique.length >= 4 && item.amount > totalSum * 0.6) return false;

      return true;
    });

    // Only use filtered if it actually removed something sensible
